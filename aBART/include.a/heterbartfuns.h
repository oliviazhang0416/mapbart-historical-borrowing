...
//--------------------------------------------------
void hetergetsuff(tree& x, tree::tree_p nx, 
                  size_t v, size_t c, 
                  xinfo& xi, dinfo& di, 
                  size_t& nl, double& bl, double& Ml, 
                  size_t& nr,  double& br, double& Mr, 
                  double *sigma, int shards)
{
   double *xx;//current x
   bl=0; Ml=0.0; 
   br=0; Mr=0.0; 
...
//--------------------------------------------------
void heterallsuff(tree& x, xinfo& xi, dinfo& di, 
                  tree::npv& bnv, 
                  std::vector<double>& bv, std::vector<double>& Mv,
                  double *sigma)
{
...
void heterdrmu(tree& t, xinfo& xi, dinfo& di, pinfo& pi, double *sigma, rn& gen)
{
   ...
   heterallsuff(t,xi,di,bnv,
                bv,Mv,
                sigma);
   
   for(tree::npv::size_type i=0;i!=bnv.size();i++){
     // cout << "Node " << i+1 << endl;
     bnv[i] -> settheta(heterdrawnodemu(bv[i],
                                        Mv[i],
                                          pi.tau,
                                          gen));
     // cout << "Mv[i] = " << Mv[i] << endl;
