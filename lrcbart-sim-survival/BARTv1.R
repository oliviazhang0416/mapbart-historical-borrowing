...
.scpp_cache <- file.path(tempdir(), "scpp_BARTv1")
...
if (hypo == "alternative") {
  threshold_file <- file.path(projectDir, "res",
    paste0("BARTv1_p", p_obs, "_", scenario_id, "_threshold.RData"))
...
# LRC-BART MODIFICATION START
sourceCpp("/Users/oliviazhang/Desktop/aBART/cabart.cpp", cacheDir = .scpp_cache)
# LRC-BART MODIFICATION END
  ...
  for (c in cor){
    for (H in ntree){
      for (replicate_id in seq_len(n_replicates)){
        ...
        #------------------------------------------------
        #----------- RCT Control only (no RWD) ----------
        #------------------------------------------------
        x.train = data.frame(X = as.matrix(data$X[data$Z == 0 & data$D == 1, ]))
        y.train = log(data$Y[data$Z == 0 & data$D == 1])
        event = as.integer(data$event[data$Z == 0 & data$D == 1])
...
saveRDS(res, file = file.path(projectDir, "res",
  paste0("BARTv1_p", p_obs, "_", scenario_id, "_", hypo, ".RData")))
# LRC-BART MODIFICATION END
