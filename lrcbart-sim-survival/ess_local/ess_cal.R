...
# Replicate-specific censored-survival LRC-BART ESS calibration.
#
...
if (ess_use_checkpoint) {
  ...
  if (!any(ess_idx_rwd) || !any(ess_idx_rct) || !any(ess_idx_rct_ctrl))
    stop("Survival two-arm calibration requires RWD, RCT profiles, and RCT controls")
  ...
  # Plain censored AFT-BART on the RCT controls supplies sigma_1^2 in RCT
  # log-time units.
  ess_rct_y <- log(as.numeric(ess_dat$y[ess_idx_rct_ctrl]))
  ess_rct_status <- as.integer(ess_dat$delta[ess_idx_rct_ctrl])
  ess_rct_x <- ess_x_all[ess_idx_rct_ctrl, , drop = FALSE]
  ess_rct_center <- mean(ess_rct_y)
  ess_rct_yc <- ess_rct_y - ess_rct_center
  ess_rct_censor <- ess_rct_y - ess_rct_center
  ess_rct_range <- diff(range(ess_rct_yc))
  ess_rct_sigma_hat <- 1
  if (length(ess_rct_yc) > ncol(ess_rct_x) + 5L) {
    ess_rct_lm <- try(survival::survreg(
      survival::Surv(exp(ess_rct_y), ess_rct_status) ~ .,
      data = data.frame(ess_rct_x), dist = "lognormal"
    ), silent = TRUE)
    if (!inherits(ess_rct_lm, "try-error") &&
        is.finite(ess_rct_lm$scale) && ess_rct_lm$scale > 0)
      ess_rct_sigma_hat <- ess_rct_lm$scale
  ...
  ess_rct_fit <- clrcbart(
    ...
    rep.int(1L, length(ess_rct_yc)), ess_rct_status, ess_rct_censor,
