...
if (hypo == "alternative") {
  threshold_file <- file.path(projectDir, "res",
    paste0("AFTv1_p", p_obs, "_", scenario_id, "_threshold.RData"))
...
for (c in cor){
  ...
  y_ctrl <- log(data$Y[data$Z == 0 & data$D == 1])
  event_ctrl <- data$event[data$Z == 0 & data$D == 1]
...
saveRDS(res, file = file.path(projectDir, "res",
  paste0("AFTv1_p", p_obs, "_", scenario_id, "_", hypo, ".RData")))
# LRC-BART MODIFICATION END
