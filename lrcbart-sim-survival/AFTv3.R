...
if (hypo == "alternative") {
  threshold_file <- file.path(projectDir, "res",
    paste0("AFTv3_p", p_obs, "_", scenario_id, "_threshold.RData"))
...
for (c in cor){
  ...
  # v3: control model includes D, so append D=1 for the target population
  x_test_ctrl <- cbind(x_test, D = 1)
  ...
  for (i in 1:n_samples) {
    mu_pred_ctrl[i, ] <- samples_ctrl$alpha[i] + x_test_ctrl %*% samples_ctrl$beta[i, ]
  }
...
saveRDS(res, file = file.path(projectDir, "res",
  paste0("AFTv3_p", p_obs, "_", scenario_id, "_", hypo, ".RData")))
# LRC-BART MODIFICATION END
