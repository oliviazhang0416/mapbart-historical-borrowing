  ...
  if (sc %in% c("1", "2", "1a", "1b", "1c", "1c-i", "1c-ii")) {
    ...
    for (configuration in selected_configurations(sc, configurations)) {
      for (c in selected_correlations(sc, cor)) {
        # Try both null and alternative
        for (hypo in plot_hypothesis) {
          # Construct filename: data_p10_sc3F_0.5_alternative_1.RData
...
# LRC-BART ADDITION START
# Create the matching covariate/log-time display for Sc4 and Sc5.
if (!is.null(sc45_long) && nrow(sc45_long) > 0) {
  sc45_long_clean <- sc45_long[
    !is.na(sc45_long$Value) & !is.na(sc45_long$y) & sc45_long$y > 0, ]

  p_cov_sc45_main <- ggplot(sc45_long_clean,
                            aes(x = Value, y = log(y), color = Group)) +
    geom_point(alpha = 0.1, size = 0.5, na.rm = TRUE) +
    geom_density_2d(alpha = 0.8, linewidth = 0.5, na.rm = TRUE) +
...
# Arrange the available plots using patchwork.
plot_parts <- list(legend_plot_final, p_km)
plot_heights <- c(0.5, km_height)
if (has_sc12_plot) {
...
save_pipeline_plot(paste0(file.path(mainDir),"/lrcbart-sim-survival/inserts/balance_all_scenarios", output_suffix, ".jpg"),
       width = total_width,
       height = total_height,
       plot,
       limitsize = FALSE)
