# Phase 2 development go/no-go run: Gaussian outcome, R = 200, seed 2026,
...
# Usage: Rscript run_dev.R   (resumable: existing RDS files are skipped)
CODE_ROOT <- normalizePath(file.path(dirname(sub("--file=", "", grep("--file=", commandArgs(), value = TRUE))), "..", "..", "01-code"))
OUT_DIR <- file.path(dirname(CODE_ROOT), "02-validation", "dev_gaussian")
...
scenarios <- c("Sc1", "Sc2", "Sc3_rho-0.5", "Sc3_rho0", "Sc3_rho0.5",
               ...
               "Sc5_d1", "Sc5_d2")
methods <- c(default_methods("gaussian"), "LRC-BART-100", "LRC-BART-75", "LRC-BART-50", "C-BART", "LRC-BART-w0")
t0 <- Sys.time()
cat("start", format(t0), "\n")
