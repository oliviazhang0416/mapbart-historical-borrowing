...
\begin{aligned}
r_{3,i}^f&\coloneqq Y_{3,i}-f^{(-h^f)}(x_{3,i}),\\
r_{2,i}^f&\coloneqq Y_{2,i}-f^{(-h^f)}(x_{2,i})-g(x_{2,i}),\\
r_{2,i}^g&\coloneqq Y_{2,i}-f(x_{2,i})-g^{(-h^g)}(x_{2,i}).
\end{aligned}
\end{equation}
The residuals with subscript $2$ use trial controls only. $f^{(-h^f)}$ and $g^{(-h^g)}$ denote the respective sums with the current tree omitted; the tree index is suppressed on the residuals. These residuals remain fixed when comparing current and proposed structures. For survival data, $Y_{3,i}$ and $Y_{2,i}$ in these calculations are the current observed or augmented log event time.
