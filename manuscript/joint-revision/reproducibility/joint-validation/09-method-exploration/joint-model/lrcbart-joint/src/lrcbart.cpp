...
//   y_i = f(x_i) + D_i g(x_i) + A_i tau + eps_i,
//   eps_i ~ N(0, sigma_{src_i}^2),
...
// Shared scalar Gaussian conditional.  Both the live sampler update and the
// exported fixed-variance reference call this function, so the reference
// checks the production algebra rather than a second copy of it.
inline std::pair<double, double> tau_moments(const std::vector<double>& yy,
                                             const std::vector<double>& aa,
                                             const std::vector<int>& ss,
                                             const std::vector<double>& base,
                                             double sigma1sq, double sigma2sq,
                                             double prior_var) {
  double prec = 1.0 / prior_var, rhs = 0.0;
  for (size_t i = 0; i < yy.size(); ++i) {
    double s2 = ss[i] == 1 ? sigma1sq : sigma2sq;
    prec += aa[i] * aa[i] / s2;
    rhs += aa[i] * (yy[i] - base[i]) / s2;
  }
  double V = 1.0 / prec;
  return std::make_pair(V * rhs, V);
}
...
public:
  ...
  std::vector<double> y;
  std::vector<double> A;        // optional treatment vector; empty disables tau
  bool has_treatment;
  ...
  // parameters
  double sig1sq, sig2sq, tau0sq, tau1sq, w, lam2, tau;
  double tau_prior_var;
  // hyperparameters
  ...
  void compute_resid(const Ensemble& E, int h) {
    if (E.isg) {
      for (int i = 0; i < n; ++i)
        resid[i] = (src[i] == 1) ? y[i] - F.tot[i] - G.tot[i] -
          (has_treatment ? A[i] * tau : 0.0) + E.fit[h][i] : 0.0;
    } else {
      for (int i = 0; i < n; ++i) {
        double gpart = (src[i] == 1 && G.H > 0) ? G.tot[i] : 0.0;
        resid[i] = y[i] - F.tot[i] + E.fit[h][i] - gpart -
          (has_treatment ? A[i] * tau : 0.0);
      }
    }
  }
  ...
  // Gibbs update for tau | f, g, sigma, y.  This is deliberately skipped when
  // treatment is disabled, preserving the original sampler's RNG stream.
  void update_tau() {
    if (!has_treatment) return;
    std::vector<double> base(n);
    for (int i = 0; i < n; ++i) {
      base[i] = F.tot[i] + ((src[i] == 1 && G.H > 0) ? G.tot[i] : 0.0);
    }
    std::pair<double, double> q = tau_moments(y, A, src, base, sig1sq, sig2sq,
                                              tau_prior_var);
    tau = q.first + std::sqrt(q.second) * norm_rand();
  }
...
List lrc_sampler_cpp(NumericMatrix X, NumericVector y, IntegerVector src,
                     ...
                     NumericVector treatment, List hyper, List control) {
  Sampler S;
  S.n = X.nrow(); S.p = X.ncol();
  S.Xv.assign(X.begin(), X.end());
  S.y.assign(y.begin(), y.end());
  S.has_treatment = treatment.size() == S.n;
  if (S.has_treatment) S.A.assign(treatment.begin(), treatment.end());
  ...
  S.tau_prior_var = hyper.containsElementNamed("tau_prior_var") ?
    as<double>(hyper["tau_prior_var"]) : 100.0;
  S.tau = 0.0;
...
// Fixed-variance conjugate reference for the treatment update.  This is an
// exported numerical check on the same N(0, 100) conditional used internally;
// it avoids using BART draws to validate the scalar Gibbs step.
// [[Rcpp::export]]
List joint_tau_draw_cpp(NumericVector y, NumericVector A, IntegerVector src,
                        NumericVector base,
                        double sigma1sq, double sigma2sq,
                        double prior_var = 100.0, int ndraw = 10000) {
  if (y.size() != A.size() || y.size() != src.size() || y.size() != base.size())
    stop("y, A, src, and base must have equal length");
  if (!R_FINITE(sigma1sq) || !R_FINITE(sigma2sq) || sigma1sq <= 0.0 ||
      sigma2sq <= 0.0 || !R_FINITE(prior_var) || prior_var <= 0.0)
    stop("variances and prior_var must be positive finite values");
  std::vector<double> yy(y.begin(), y.end()), aa(A.begin(), A.end());
  std::vector<double> bb(base.begin(), base.end());
  std::vector<int> ss(src.begin(), src.end());
  for (int i = 0; i < src.size(); ++i) {
    if (src[i] != 1 && src[i] != 2) stop("src must contain only 1 or 2");
    if (!R_FINITE(yy[i]) || !R_FINITE(aa[i]) || !R_FINITE(bb[i]))
      stop("y, A, and base must be finite");
  }
  std::pair<double, double> q = tau_moments(yy, aa, ss, bb, sigma1sq,
                                            sigma2sq, prior_var);
  double m = q.first, V = q.second;
  NumericVector draws(ndraw);
  for (int d = 0; d < ndraw; ++d) draws[d] = m + std::sqrt(V) * norm_rand();
  return List::create(Named("mean") = m, Named("variance") = V,
                      Named("draws") = draws);
}
