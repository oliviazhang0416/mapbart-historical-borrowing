...
lrc_core <- function(y, x, source, status = NULL, aft = FALSE,
                     ...
                     treatment = NULL, tau_prior_var = 100) {
  x <- as_x_matrix(x)
  n <- nrow(x)
  y <- as.numeric(y)
  ...
  if (!is.null(treatment)) {
    if (length(treatment) != n || any(!is.finite(treatment)))
      stop("treatment must be a finite vector with length nrow(x)")
    if (any(treatment[source == 2L] != 0))
      stop("treatment must be zero for source 2 observations")
    if (length(tau_prior_var) != 1L || !is.finite(tau_prior_var) || tau_prior_var <= 0)
      stop("tau_prior_var must be a positive finite scalar")
  }
  ...
  hyper$tau_prior_var <- tau_prior_var
  out <- lrc_sampler_cpp(x, yc, source, status, logC, cutpoints,
                         if (is.null(treatment)) numeric(0) else as.numeric(treatment),
                         hyper, control)
  elapsed <- proc.time()[["elapsed"]] - t0

  fit <- list(
    draws = list(f = out$f_draws, g = out$g_draws, tau = out$tau),
...
predict.lrcbart <- function(object, x_new, arm = c("rct_control", "rwd_control"),
  ...
  ans <- if (arm == "rwd_control" || object$H_g == 0) f else
    f + predict_forest_cpp(object$draws$g, x_new)$value
  if (!is.null(treatment)) {
    if (length(treatment) != nrow(x_new) || any(!is.finite(treatment)))
      stop("treatment must be a finite vector with length nrow(x_new)")
    if (is.null(object$draws$tau)) stop("fit has no treatment draws")
    ans <- ans + outer(object$draws$tau, as.numeric(treatment), `*`)
  }
  ans
}

# Experimental joint treatment model.  With treatment = NULL this is exactly
# the original LRC-BART model, including its random-number path.  A supplied
# treatment vector adds A_i * tau with tau ~ N(0, tau_prior_var).
joint_lrc_bart <- function(y, x, source, treatment = NULL, ...) {
  lrc_core(y = y, x = x, source = source, treatment = treatment, ...)
}

# Joint source-BART comparison: source is represented by an appended binary
# covariate and the explicit g ensemble is disabled.  Residual variances remain
# source-specific through the same sampler as LRC-BART.
joint_source_bart <- function(y, x, source, treatment = NULL, ...) {
  x <- as_x_matrix(x)
  source <- as.integer(source)
  if (length(source) != nrow(x) || any(!source %in% c(1L, 2L)))
    stop("source must be a vector of 1/2 with length nrow(x)")
  xa <- cbind(x, source_rct = as.numeric(source == 1L))
  lrc_core(y = y, x = xa, source = source, H_g = 0,
           treatment = treatment, ...)
}

# Closed-form conditional for the Gaussian coefficient used by the sampler.
# This is also the analytic reference used by the exploratory validation.
joint_tau_conditional <- function(y, A, source, sigma1_sq, sigma2_sq,
                                  prior_var = 100) {
  y <- as.numeric(y); A <- as.numeric(A); source <- as.integer(source)
  if (length(y) != length(A) || length(y) != length(source) ||
      any(!is.finite(y)) || any(!is.finite(A)) ||
      any(!source %in% c(1L, 2L))) stop("invalid tau conditional inputs")
  if (length(sigma1_sq) != 1L || length(sigma2_sq) != 1L ||
      !is.finite(sigma1_sq) || !is.finite(sigma2_sq) ||
      sigma1_sq <= 0 || sigma2_sq <= 0 || prior_var <= 0)
    stop("variances and prior_var must be positive finite scalars")
  s2 <- ifelse(source == 1L, sigma1_sq, sigma2_sq)
  V <- 1 / (1 / prior_var + sum(A^2 / s2))
  list(mean = V * sum(A * y / s2), variance = V)
}

joint_tau_reference_draws <- function(y, A, source, sigma1_sq, sigma2_sq,
                                      prior_var = 100, n = 10000, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  q <- joint_tau_conditional(y, A, source, sigma1_sq, sigma2_sq, prior_var)
  list(draws = rnorm(n, q$mean, sqrt(q$variance)), conditional = q)
}
