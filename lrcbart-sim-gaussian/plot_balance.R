...
dummy_data <- data.frame(
  x = rep(1, 3),
  Group = factor(c("Control", "External Control", "Treatment"),
                 levels = c("Control", "External Control", "Treatment"))
)
...
save_pipeline_plot(paste0(file.path(mainDir),"/lrcbart-sim-gaussian/inserts/balance_all_scenarios", output_suffix, ".jpg"),
       width = total_width,
       height = total_height,
       plot,
       limitsize = FALSE)
