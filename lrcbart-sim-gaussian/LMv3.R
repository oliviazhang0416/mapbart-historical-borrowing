...
if (hypo == "alternative") {
  threshold_file <- file.path(projectDir, "res",
    paste0("LMv3_p", p_obs, "_", scenario_id, "_threshold.RData"))
...
for (c in cor){
  for (replicate_id in seq_len(n_replicates)){
    ...
    # Predict for target population (D=1), setting the data source indicator to D=1
    x_test <- as.matrix(data$X[data$D == 1, ])
    x_test <- scale(x_test, center = x_means, scale = FALSE)
    x_test <- cbind(x_test, D = 1)
    ...
    # Predict for target population (D=1); treatment model has no D column
    x_test <- as.matrix(data$X[data$D == 1, ])
    x_test <- scale(x_test, center = x_means, scale = FALSE)
...
saveRDS(res, file = file.path(projectDir, "res",
  paste0("LMv3_p", p_obs, "_", scenario_id, "_", hypo, ".RData")))
# LRC-BART MODIFICATION END
