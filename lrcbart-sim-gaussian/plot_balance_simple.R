...
# The inherited Sc1-Sc3 simple balance plot is extended with the revision's
# Sc4 global-shift and Sc5 regional-partial-compatibility configurations.
mainDir <- .lrcRoot
data_folder <- "data"
    ...
    for (configuration in selected_configurations(sc, configurations)) {
      for (c in selected_correlations(sc, cor)) {
        for (hypo in plot_hypothesis) {
          ...
          filename_rct <- paste0(mainDir,"/lrcbart-sim-gaussian/",data_folder,
                                 "/data_p",p_obs,"_",scenario_tag,"_",hypo,
                                 "_",iter_rct,".RData")
          filename_rwd <- paste0(mainDir,"/lrcbart-sim-gaussian/",data_folder,
                                 "/data_p",p_obs,"_",scenario_tag,"_",hypo,
                                 "_",iter_rwd,".RData")
      ...
      for (c in selected_correlations(sc, cor)) {
        for (hypo in plot_hypothesis) {
          if (configuration == "Base") {
            filename_rct <- paste0(mainDir,"/lrcbart-sim-gaussian/",data_folder,"/data_p",p_obs,"_sc",sc,"_cor",c,"_",hypo,"_",iter_rct,".RData")
            filename_rwd <- paste0(mainDir,"/lrcbart-sim-gaussian/",data_folder,"/data_p",p_obs,"_sc",sc,"_cor",c,"_",hypo,"_",iter_rwd,".RData")
          } else {
            filename_rct <- paste0(mainDir,"/lrcbart-sim-gaussian/",data_folder,"/data_p",p_obs,"_sc",sc,configuration,"_cor",c,"_",hypo,"_",iter_rct,".RData")
            filename_rwd <- paste0(mainDir,"/lrcbart-sim-gaussian/",data_folder,"/data_p",p_obs,"_sc",sc,configuration,"_cor",c,"_",hypo,"_",iter_rwd,".RData")
          }
...
all_data$Scenario_Label <- ifelse(
  ...
  ifelse(all_data$Scenario %in% c("1a", "1b", "1c", "1c-i", "1c-ii"),
...
# Create density plots for each scenario configuration.
outcome_plots <- list()
...
p_outcome_final <- wrap_plots(outcome_plots, ncol = 4)

# Save outcome density plot
output_suffix <- ifelse(data_folder == "data", "", paste0("_", data_folder))
save_pipeline_plot(paste0(file.path(mainDir),"/lrcbart-sim-gaussian/inserts/balanced_1.jpg"),
       width = 16,
       height = 3 * ceiling(length(outcome_plots) / 4),
       p_outcome_final,
       limitsize = FALSE)
...
save_pipeline_plot(paste0(file.path(mainDir),"/lrcbart-sim-gaussian/inserts/balanced_2.jpg"),
       width = 24,
       height = plot_height,
       p_balance,
       limitsize = FALSE)
