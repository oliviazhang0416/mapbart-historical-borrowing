...
# Joint result and balance readers support all Sc1 variants.
...
# Default: full grid. Example: Rscript run_all.R --scenarios sc1c,c-i,c-ii
# Both --scenarios LIST and --scenarios=LIST accept comma-separated labels.
...
data_configs <- list(
  ...
  list(sc = 3, variant = "", delta_rwd = 0, region = "none")
)
analysis_scenarios <- list(
  ...
  list(sc = 3, variant = "", cor = -0.5, delta_rwd = 0, region = "none"),
  list(sc = 3, variant = "", cor = 0, delta_rwd = 0, region = "none"),
  list(sc = 3, variant = "", cor = 0.5, delta_rwd = 0, region = "none")
)
# JOINT LRC-BART MODIFICATION END
...
if (!is.null(selected_scenarios)) {
  invalid_scenarios <- setdiff(selected_scenarios,
    c("sc1", "sc1a", "sc1b", "sc1c", "sc1c-i", "sc1c-ii", "sc2", "sc3"))
...
method_overrides <- list(
  ...
  # JOINT LRC-BART ADDITION START
  "lrcBART.R" = list(n_chains = 1L, joint_n_burn = 1000L,
    joint_n_draw = 1000L, joint_thin = 1L, g_sweeps = 1L,
...
run_simulation_pipeline <- function() {
  ...
  # Required hypotheses are explicit; ordinary scenarios follow hypo_values.
