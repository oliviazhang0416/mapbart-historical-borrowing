...
#
#   1. Generate data    via  data_gen_p<p_obs>.R
#   2. Plot balance     via  plot_balance.R and plot_balance_simple.R
#   3. Run analyses     via  LMv1, LMv2, LMv3, hierLM, MAP, PSCL, BARTv1, BARTv2, BARTv3, lrcBART
#   4. Plot ESS         via  ess_local/ess_plot.R
#   5. Plot results     via  plot.R
#
# The master parses each file in memory, replaces matching assignments with
# the current overrides, and evaluates it in an isolated environment.
# The source files on disk are never modified during a pipeline run.
#
...
mainDir <- .lrcRoot
script_dir <- file.path(mainDir, "lrcbart-sim-gaussian")
...
run_plot         <- TRUE
# LRC-BART MODIFICATION START
# Add fixed-w target-100 fits after the standard LRC-BART fit in Sc1-Sc3.
# Hg10 remains available in lrcBART.R but is not selected here.
run_lrc_sensitivities <- TRUE
sensitivity_configs <- c("w1", "w0")
sensitivity_scenarios <- 1:3
# LRC-BART MODIFICATION END
...
# The generator owns its correlation and replicate loops. These are source
# configurations, not replicate loops or fitting work.
data_configs <- list(
  list(sc = 1, delta_rwd = 0, region = "none"),
  list(sc = 2, delta_rwd = 0, region = "none"),
  list(sc = 3, delta_rwd = 0, region = "none"),
  list(sc = 4, delta_rwd = 1, region = "none"),
  list(sc = 4, delta_rwd = 2, region = "none"),
  list(sc = 5, delta_rwd = 0.5, region = "X5"),
  list(sc = 5, delta_rwd = 1, region = "X5"),
  list(sc = 5, delta_rwd = 2, region = "X5"),
  list(sc = 5, delta_rwd = 1, region = "X7"),
  list(sc = 5, delta_rwd = 2, region = "X7")
)
analysis_scenarios <- list(
  list(sc = 1, cor = 1, delta_rwd = 0, region = "none"),
  list(sc = 2, cor = 1, delta_rwd = 0, region = "none"),
  list(sc = 3, cor = -0.5, delta_rwd = 0, region = "none"),
  list(sc = 3, cor = 0, delta_rwd = 0, region = "none"),
  list(sc = 3, cor = 0.5, delta_rwd = 0, region = "none"),
  list(sc = 4, cor = 1, delta_rwd = 1, region = "none"),
  list(sc = 4, cor = 1, delta_rwd = 2, region = "none"),
  list(sc = 5, cor = 1, delta_rwd = 0.5, region = "X5"),
  list(sc = 5, cor = 1, delta_rwd = 1, region = "X5"),
  list(sc = 5, cor = 1, delta_rwd = 2, region = "X5"),
  list(sc = 5, cor = 1, delta_rwd = 1, region = "X7"),
  list(sc = 5, cor = 1, delta_rwd = 2, region = "X7")
)
# LRC-BART ADDITION END

# ---- Analysis files ----
# LRC-BART MODIFICATION START
files <- c("LMv1.R", "LMv2.R", "LMv3.R", "BARTv1.R", "BARTv2.R",
           "BARTv3.R", "PSCL.R", "MAP.R", "hierLM.R", "lrcBART.R")
# LRC-BART MODIFICATION END
# Files in `serial_files` run sequentially AFTER the parallel batch (use for
# methods whose native libraries aren't fork-safe). rstan (and RBesT, which is
# built on rstan) breaks when several Stan models are mcparallel-forked at once
# -> "invalid connection". LRC-BART also compiles and loads its standalone C++
# sources, which must happen in the main R process rather than a forked worker.
# The Stan/RBesT and LRC-BART methods therefore run serially; only the three
# inherited BART comparators go in the parallel batch.
# LRC-BART MODIFICATION START
serial_files <- c("LMv1.R", "LMv2.R", "LMv3.R", "hierLM.R", "MAP.R",
                  "PSCL.R", "lrcBART.R")
# LRC-BART MODIFICATION END

# ---- Seeds ----
# seed_rct      : data_gen RCT replicate seed-array generator (rewrites set.seed(<literal>))
...
#
# LRC-BART MODIFICATION START
# MAP.R and lrcBART.R own their target loops and local ESS calls.
method_overrides <- list(
  "hierLM.R" = list(prior_vals = 0.25),
  "MAP.R" = list(target_Ns = c(100L, 75L, 50L))
)
# LRC-BART MODIFICATION END
# Extra data-generation overrides.
data_gen_overrides <- list(
  seed_rct    = seed_rct,  # RCT replicate seed-array generator (takes precedence over seed_method here)
  seed_rwd    = seed_rwd,  # RWD-pool seed (overrides `seed_rwd <- ...` in data_gen)
  rwd_frozen  = FALSE,     # fresh RWD per replicate (un-suffixed files); TRUE/omit = frozen pool (_fz)
  p_obs = p_obs
)
...
run_simulation_pipeline <- function() {
  ...
  plot_context_for <- function(hypothesis, size, scenarios) list(
    ...
    lrc_sensitivity_scenarios = sensitivity_scenarios,
  ...
  if (run_analysis) {
    for (size in sizes) for (hypothesis in hypo_values) for (cfg in analysis_scenarios) {
      ...
      mark_failed(result)
      if (run_lrc_sensitivities && cfg$sc %in% sensitivity_scenarios &&
          "lrcBART.R" %in% files) {
