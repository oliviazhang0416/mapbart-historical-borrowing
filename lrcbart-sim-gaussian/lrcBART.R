...
# run_all.R leaves this at "default" for the six target fits and overrides it
# with one of w0, w1, or Hg10 for a sensitivity invocation.
lrc_config <- "default"
...
for (replicate_id in seq_len(n_replicates)) {
  ...
  for (target_index in seq_along(target_names)) {
    ...
    if (length(y_sigma1) > ncol(x_sigma1) + 5L) {
      sigma_fit1 <- try(lm(
        y_sigma1 ~ .,
        data = data.frame(y_sigma1 = y_sigma1, x_sigma1)
      ), silent = TRUE)
      if (!inherits(sigma_fit1, "try-error") &&
          is.finite(summary(sigma_fit1)$sigma) &&
          summary(sigma_fit1)$sigma > 0)
    ...
    if (length(y_sigma2) > ncol(x_sigma2) + 5L) {
      sigma_fit2 <- try(lm(
        y_sigma2 ~ .,
        data = data.frame(y_sigma2 = y_sigma2, x_sigma2)
      ), silent = TRUE)
      if (!inherits(sigma_fit2, "try-error") &&
          is.finite(summary(sigma_fit2)$sigma) &&
          summary(sigma_fit2)$sigma > 0)
