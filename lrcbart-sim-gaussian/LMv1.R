...
if (hypo == "alternative") {
  threshold_file <- file.path(projectDir, "res",
    paste0("LMv1_p", p_obs, "_", scenario_id, "_threshold.RData"))
...
for (c in cor){
  for (replicate_id in seq_len(n_replicates)){
    ...
    x_ctrl <- as.matrix(data$X[data$Z == 0 & data$D == 1, ])
    x_ctrl <- scale(x_ctrl, center = x_means, scale = FALSE)
    y_ctrl <- data$Y[data$Z == 0 & data$D == 1]
    
    # Estidelta_hat sigma using linear model (BART approach)
    nu <- 3
    sigquant <- 0.90
    ...
    # Posterior estidelta_hats per subject
...
saveRDS(res, file = file.path(projectDir, "res",
  paste0("LMv1_p", p_obs, "_", scenario_id, "_", hypo, ".RData")))
# LRC-BART MODIFICATION END
