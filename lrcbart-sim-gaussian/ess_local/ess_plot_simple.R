...
# Minimal companion to ess_plot.R for one replicate-specific LRC-BART
# calibration checkpoint. It reads saved calibration results only.
...
if (is.null(cal$ess_tau0_grid) || is.null(cal$grid))
  stop("Checkpoint does not contain ess_tau0_grid and grid: ", cal_path)

mean_data <- data.frame(s0_sq = cal$grid, ESS_tau0 = cal$ess_tau0_grid)
selected <- cal$targets[, c("target_name", "s0_sq", "ess_tau0")]
p <- ggplot2::ggplot(mean_data, ggplot2::aes(x = s0_sq, y = ESS_tau0)) +
...
ggplot2::ggsave(out_path, p, width = 6.5, height = 4.2)
cat("Wrote", out_path, "\n")
