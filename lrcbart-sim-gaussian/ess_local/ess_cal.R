...
# Replicate-specific LRC-BART ESS calibration.
#
...
if (ess_use_checkpoint) {
  # Read checkpoints created before ESS_tau0 replaced the ambiguous ESS0 name.
  if (is.null(ess_cached$targets$ess_tau0) &&
      !is.null(ess_cached$targets$ess0))
    ess_cached$targets$ess_tau0 <- ess_cached$targets$ess0
  if (is.null(ess_cached$ess_tau0_blocks) &&
      !is.null(ess_cached$ess0_blocks))
    ess_cached$ess_tau0_blocks <- ess_cached$ess0_blocks
  if (is.null(ess_cached$ess_tau0_grid) &&
      !is.null(ess_cached$ess0_grid))
    ess_cached$ess_tau0_grid <- ess_cached$ess0_grid
  ...
  ess_idx_rct <- ess_d == 1
  ess_idx_rct_ctrl <- ess_d == 1 & ess_z == 0
  if (!any(ess_idx_rwd) || !any(ess_idx_rct) || !any(ess_idx_rct_ctrl))
    stop("Gaussian two-arm calibration requires RWD, RCT profiles, and RCT controls")

  # Stage 1: plain BART on the RWD controls, evaluated at all RCT profiles.
  ess_stage1_y <- as.numeric(ess_dat$y[ess_idx_rwd])
  ...
  if (length(ess_stage1_yc) > ncol(ess_stage1_x) + 5L) {
    ess_stage1_lm <- try(stats::lm(
      ess_stage1_yc ~ .,
      data = data.frame(ess_stage1_yc = ess_stage1_yc, ess_stage1_x)
    ), silent = TRUE)
    if (!inherits(ess_stage1_lm, "try-error") &&
        is.finite(summary(ess_stage1_lm)$sigma) &&
        summary(ess_stage1_lm)$sigma > 0)
      ess_stage1_sigma_hat <- summary(ess_stage1_lm)$sigma
  ...
  ess_stage1_fit <- clrcbart(
    ...
    rep.int(2L, length(ess_stage1_yc)), integer(0), numeric(0),
  ...
  # Plain BART on the RCT controls supplies sigma_1^2 in RCT units.
  ess_rct_y <- as.numeric(ess_dat$y[ess_idx_rct_ctrl])
  ess_rct_x <- ess_x_all[ess_idx_rct_ctrl, , drop = FALSE]
  ess_rct_center <- mean(ess_rct_y)
  ess_rct_yc <- ess_rct_y - ess_rct_center
  ess_rct_range <- diff(range(ess_rct_yc))
  ess_rct_sigma_hat <- 1
  if (length(ess_rct_yc) > ncol(ess_rct_x) + 5L) {
    ess_rct_lm <- try(stats::lm(
      ess_rct_yc ~ .,
      data = data.frame(ess_rct_yc = ess_rct_yc, ess_rct_x)
    ), silent = TRUE)
    if (!inherits(ess_rct_lm, "try-error") &&
        is.finite(summary(ess_rct_lm)$sigma) &&
        summary(ess_rct_lm)$sigma > 0)
      ess_rct_sigma_hat <- summary(ess_rct_lm)$sigma
  } else if (is.finite(stats::sd(ess_rct_yc)) &&
             stats::sd(ess_rct_yc) > 0) {
    ess_rct_sigma_hat <- stats::sd(ess_rct_yc)
  }
  if (!is.finite(ess_rct_range) || ess_rct_range <= 0)
    ess_rct_range <- 2 * ess_rct_sigma_hat
  ess_rct_cutpoints <- vector("list", ncol(ess_rct_x))
  for (ess_j in seq_len(ncol(ess_rct_x))) {
    ess_u <- sort(unique(ess_rct_x[, ess_j]))
    if (length(ess_u) < 2L)
      stop("LRC-BART requires at least two observed values for predictor ", ess_j)
    if (length(ess_u) <= 100L) {
      ess_rct_cutpoints[[ess_j]] <-
        (ess_u[-1] + ess_u[-length(ess_u)]) / 2
    } else {
      ess_r <- range(ess_u)
      ess_rct_cutpoints[[ess_j]] <-
  ...
  ess_rct_hyper <- list(
    H_f = as.integer(ess_H_f), H_g = 0L,
    alpha_f = 0.95, beta_f = 2, alpha_g = 0.5, beta_g = 3,
    lambda_f_sq = (ess_rct_range / (4 * sqrt(ess_H_f)))^2,
    nu_sigma = 3,
    lambda_sigma1 = ess_rct_sigma_hat^2 * stats::qchisq(0.10, 3) / 3,
    lambda_sigma2 = ess_rct_sigma_hat^2 * stats::qchisq(0.10, 3) / 3,
    nu0 = 3, s0_sq = 1, tau1_sq = 1,
    a_w = 1, b_w = 1, update_tau0 = FALSE,
    augmentation = FALSE, p_grow = 0.3, p_prune = 0.3,
    n_min = 5L, n_min_g = 0L,
    sigma1_sq_init = ess_rct_sigma_hat^2,
    sigma2_sq_init = ess_rct_sigma_hat^2,
    w_fixed = NA_real_
  )
  ess_rct_control <- list(
    n_burn = as.integer(ess_n_burn), n_draw = as.integer(ess_n_draw),
    thin = 1L, verbose = FALSE, keep_trees = FALSE, keep_train = FALSE,
    keep_test = TRUE, seed = as.integer(ess_seed + 11L)
  )
  ess_rct_fit <- clrcbart(
    t(ess_rct_x), ess_rct_yc,
    rep.int(1L, length(ess_rct_yc)), integer(0), numeric(0),
    t(ess_rct_x), ess_rct_cutpoints, ess_rct_hyper, ess_rct_control
  )

  ess_mu_draws <- rowMeans(ess_stage1_f_test)
  ess_V_whole <- stats::var(ess_mu_draws)
  if (!is.finite(ess_V_whole) || ess_V_whole <= 0)
    stop("Stage-1 standardized mean has non-positive variance")
  ...
  ess_sigma1_sq <- mean(ess_rct_fit$sigma1_sq)
  ...
  ess_requested <- c(100, 75, 50,
                     0.90 * ess_ceiling,
                     0.50 * ess_ceiling,
                     0.25 * ess_ceiling)
  ess_fraction <- c(NA_real_, NA_real_, NA_real_, 0.90, 0.50, 0.25)
