...
# checkpoints written by ess_cal.R and ess_cal_map.R. It performs no fitting.
...
# Overlay the matching inherited MAP calibration when it is available.
map_path <- file.path(
  resDir,
  paste0("map_ess_", tools::file_path_sans_ext(basename(cal$data_file)),
         "_checkpoint.RData")
)
if (file.exists(map_path)) {
  map <- readRDS(map_path)
  p <- p + ggplot2::geom_line(
    data = data.frame(s0_sq = map$S_grid, ESS_tau0 = map$ESS),
    ggplot2::aes(x = s0_sq, y = ESS_tau0, linetype = "MAP"),
    inherit.aes = FALSE, linewidth = 0.8, colour = "#D55E00"
  ) +
    ggplot2::scale_linetype_manual(
      name = "Curve", values = c(MAP = "solid")
    ) +
    ggplot2::guides(
      linetype = ggplot2::guide_legend(
        override.aes = list(colour = "#D55E00", linewidth = 0.8)
      )
    )
}
