...
if (hypo == "alternative") {
  threshold_file <- file.path(projectDir, "res",
    paste0("hierLM_p", p_obs, "_", scenario_id, "_threshold.RData"))
...
for (c in cor){
  for (replicate_id in seq_len(n_replicates)){
    ...
    # Control group (uses RCT variance, group_idx=2)
    sigma_samples_ctrl <- sqrt(samples_ctrl$sigma2[, 2])
  ...
  saveRDS(res, file = file.path(projectDir, "res",
    paste0("hierLM_p", p_obs, "_", scenario_id, "_prior", prior,
           "_", hypo, ".RData")))
  # LRC-BART MODIFICATION END
