...
# Replicate-specific Gaussian single-arm LRC-BART ESS calibration.
#
...
if (file.exists(ess_checkpoint) &&
  ...
  ess_use_checkpoint <- is.list(ess_cached) && identical(ess_cached$settings, ess_settings) &&
    ...
    all(c("100", "f90", "s0min") %in% names(ess_cached$s0_sq))
}
...
if (ess_use_checkpoint) {
  ...
  if (!any(ess_idx_rwd) || !any(ess_idx_rct))
    stop("Gaussian single-arm calibration requires RWD controls and RCT profiles")
  if (any(ess_d == 1 & ess_z == 0))
    stop("Gaussian single-arm calibration received RCT controls")
  ...
  # No RCT controls exist in the single-arm design. The standalone sampler
  # therefore uses the RWD residual variance for both sigma_1^2 and sigma_2^2.
  ...
  # `s0min` is a fixed-prior benchmark. It bypasses target selection and uses
  # the minimum supplied s0^2 grid value, 1e-6.
  ...
  for (ess_k in seq_along(ess_target_names)) {
    if (ess_target_names[ess_k] == "s0min") {
      # The reported near-complete-pooling reference is fixed at the smallest
      # calibration-grid value rather than selected by the probability rule.
      ess_pick <- 1L
