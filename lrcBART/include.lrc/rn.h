...
// Final revision update for tau0^2:
// scaled-Inv-chi-square(nu, s^2), restricted to (0, upper).  If
// X~chi-square(nu), tau=nu*s^2/X; tau<upper is equivalent to
// X>nu*s^2/upper.  Inverse-tail sampling avoids a rejection loop.
// ---------------------------------------------------------------------------
double rsinvchisq_upper(double nu, double ssq, double upper, rn& gen)
{
  const double cutoff=nu*ssq/upper;
  const double tail=R::pchisq(cutoff,nu,0,0);
  if(tail<=0.0) return upper*(1.0-1e-12);
  double u=gen.uniform()*tail;
  if(u<=0.0) u=std::numeric_limits<double>::min();
  const double x=R::qchisq(u,nu,0,0);
  if(!R_FINITE(x) || x<=0.0) return upper*(1.0-1e-12);
  return nu*ssq/x;
}
