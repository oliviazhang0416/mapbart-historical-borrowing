...
threshold_file <- file.path(
  resultDir,
  paste0("BARTv2_p", p_obs, size_suffix, "_", scenario_id,
         "_threshold.RData")
)
...
# LRC-BART MODIFICATION START
sourceCpp("/Users/oliviazhang/Desktop/aBART/cabart.cpp",
          cacheDir = .scpp_cache)
# LRC-BART MODIFICATION END
  ...
  for (c in cor){
    for (H in ntree){
      for (iter in 1:n_replicates){
        ...
        upper_default <- pmin(exp(apply(mu_draws, 1, max) + 6 * sig_draw), 30)
        ...
        median_survival_ctrl <- pop_median_draws(res_ctrl$yhat.test, res_trt$sigma)
...
result_file <- file.path(
  resultDir,
  paste0("BARTv2_p", p_obs, size_suffix, "_", scenario_id, "_",
         hypo, ".RData")
)
